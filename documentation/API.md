...
[Babel](https://github.com/NCATSTranslator/Babel) pipeline in its Synonyms format, including lists of all known synonyms.
See [Where NameRes data comes from](./Babel.md) for what Babel puts in these documents, which Babel files a NameRes
instance is built from, and the behaviour that follows from both.
...
all the Babel conflations are baked in when the Solr database is built. At the moment, this includes:

* **GeneProtein conflation:** protein-encoding genes are conflated with the protein(s) they encode, and the gene
  identifier is used to identify this concept. The `NCBIGene:1756` document shown above is an example: it is a gene, but
  its `names` include protein synonyms such as `dystrophin` and `DMD_HUMAN Dystrophin (sprot)`, and there is no separate
  document for the protein itself.
...
See [Where NameRes data comes from](./Babel.md#geneproteinconflated-and-drugchemicalconflated-replace-the-per-type-files)
for why conflation is baked into the index rather than applied per query, and what each conflation merges.
