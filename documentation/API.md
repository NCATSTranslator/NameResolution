...
all the Babel conflations are turned on when the Solr database is built. At the moment, this includes:

* GeneProtein conflation: protein-encoding genes are conflated with the protein(s) they encode, and the gene identifier
  is used to identify this concept. The `NCBIGene:1756` document shown above is an example: it is a gene, but its
  `names` include protein synonyms such as `dystrophin` and `DMD_HUMAN Dystrophin (sprot)`, and there is no separate
