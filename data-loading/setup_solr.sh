...
# add collection
curl -X POST "http://$SOLR_SERVER/solr/admin/collections?action=CREATE&name=name_lookup&numShards=1&replicationFactor=1"

# do not autocreate fields
curl "http://$SOLR_SERVER/solr/name_lookup/config" -d '{"set-user-property": {"update.autoCreateFields": "false"}}'
...
}' "http://$SOLR_SERVER/solr/name_lookup/schema"
